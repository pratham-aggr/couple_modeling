...
class UNet(nn.Module):
    ...
    """
    def __init__(self, n_in=3, n_out=5, base=64):
        super().__init__()
        b = base
        ...
        self.enc = nn.ModuleList([
            UNetBlock(b,    b*2,  transposed=False, bn=True, relu=False, kernel=4, pad=1),
            UNetBlock(b*2,  b*2,  transposed=False, bn=True, relu=False, kernel=4, pad=1),
            UNetBlock(b*2,  b*4,  transposed=False, bn=True, relu=False, kernel=4, pad=1),
            UNetBlock(b*4,  b*8,  transposed=False, bn=True, relu=False, kernel=4, pad=1),
            UNetBlock(b*8,  b*8,  transposed=False, bn=True, relu=False, kernel=2, pad=0),
        ])

        # Decoder (ReLU, BN); input channels doubled by skip concatenation
        self.dec = nn.ModuleList([
            UNetBlock(b*8,       b*8, transposed=True,  bn=True, relu=True, kernel=2, pad=0),
            UNetBlock(b*8 + b*8, b*4, transposed=True,  bn=True, relu=True, kernel=4, pad=1),
            UNetBlock(b*4 + b*4, b*2, transposed=True,  bn=True, relu=True, kernel=4, pad=1),
            UNetBlock(b*2 + b*2, b*2, transposed=True,  bn=True, relu=True, kernel=4, pad=1),
            UNetBlock(b*2 + b*2, b,   transposed=True,  bn=True, relu=True, kernel=4, pad=1),
        ])
...
def masked_mse(pred, target, mask):
    """MSE over ocean/ice points only. mask: (B, H, W) float32."""
    m = mask.unsqueeze(1)                                  # (B, 1, H, W)
    err = ((pred - target) ** 2) * m
    n   = m.sum() * pred.shape[1]
    return err.sum() / (n + 1e-8)
...
def main():
    ...
    parser.add_argument("--memory", action="store_true",
                        help="Memory experiment: X = [state_t, state_{t-24h}] → Y = fluxes_t. "
                             "Uses couple_cache_mem24h (6 channels stored; training selects subset).")
    parser.add_argument("--prev_solin", action="store_true",
                        help="Include SOLIN from 24h ago as an input channel (only with --memory).")
    ...
    input_vars = input_vars_base + (["CO2"] if args.with_co2 else [])
    ...
    if use_wandb:
        ...
        run_name = args.wandb_name or (
            ...
            f"{'-mem24h' if args.memory else f'-lag{args.lag}h'}"
    ...
    if cache_ok:
        ...
        if args.with_co2:
            X_all = [np.concatenate([
                        X_np[i][mem_channels] if args.memory else X_np[i],
                        np.full((1, H, W), co2_np[i], dtype=np.float32)
                     ], axis=0).copy() for i in chosen]
        elif args.memory:
            X_all = [X_np[i][mem_channels].copy() for i in chosen]
        else:
            X_all = [X_np[i].copy() for i in chosen]
    ...
    # --- Model ---
    model = UNet(n_in=len(input_vars), n_out=n_out, base=args.base).to(device)
    n_params = sum(p.numel() for p in model.parameters())
    print(f"\nUNet: {n_params:,} parameters  (base={args.base}, "
          f"n_in={len(input_vars)}, n_out={n_out})")
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        with torch.no_grad():
            for x_n, y_n, mask in val_loader:
                ...
                mask = mask.to(device)
                pred = model(x_n)
                val_loss += masked_mse(pred, y_n, mask).item()
                # Per-variable: MSE over ocean points for each channel
                m = mask.unsqueeze(1)                       # (B,1,H,W)
                n_pts = m.sum() + 1e-8
                for vi in range(n_out):
                    val_loss_pv[vi] += (((pred[:,vi:vi+1] - y_n[:,vi:vi+1])**2) * m).sum().item() / n_pts.item()
