...
def main():
    ...
    def scale_wind(a, mo, target_speed):
        """return a copy of clim atm-block with U,V rescaled to target_speed (dir kept)."""
        out = a.copy()
        sp = np.maximum(M["spdclim"][mo], 1e-6)
        sc = target_speed / sp
        out[:,0] *= sc; out[:,1] *= sc
        return out
    ...
    for i in range(0, len(rows), B):
        ...
        a_clim = clim[mo].copy()
        rms = M["rms"][mo]; meanspd = M["meanspd"][mo]

        a_gust = scale_wind(a_clim, mo, rms)
        a_gQ   = add_humcov(scale_wind(a_clim, mo, rms), mo, rms, do_T=False)
        a_gQT  = add_humcov(scale_wind(a_clim, mo, rms), mo, rms, do_T=True)
        a_mw   = add_humcov(scale_wind(a_clim, mo, meanspd), mo, meanspd, do_T=True)
