#!/usr/bin/env python
"""GLOBAL-mean SST by year: coupled ML-POP2 sim vs CESM2-LE truth (LE2-1231.002).
One point per completed calendar year (area-weighted annual mean over the full
gx1v7 ocean grid, TAREA-weighted). Re-runnable — only full calendar years present
in the run directory are plotted, so re-running as the simulation advances extends
the figure automatically.

Usage:  python plot_sst_annual.py [--run DIR] [--out PNG] [--ref-member ID]
"""
...
ap.add_argument("--out", default="/glade/u/home/praggarwal/couple/output/sst_annual.png")
...
C_SIM, C_UFT, C_UFTLOOP, C_TRUTH, C_CAM6, MUTE = "#D55E00", "#009E73", "#CC79A7", "#5b6167", "#0072B2", "#6b7177"
...
# ---------- unfinetuned model global-mean SST, its own available full years ----------
uft_years, uft_annual = None, None
uft_files = sorted(glob.glob(f"{args.run_uft}/*.pop.h.[0-9][0-9][0-9][0-9]-[0-9][0-9].nc"))
if uft_files:
    uft_keys = [ym(f) for f in uft_files]
    uft_by_ym = {k: gmean(xr.open_dataset(f)["TEMP"].isel(time=0, z_t=0).values)
                 for f, k in zip(uft_files, uft_keys)}
    uft_years_avail = sorted({y for (y, m) in uft_keys})
    uft_years = [y for y in uft_years_avail if all((y, m) in uft_by_ym for m in range(1, 13))]
    if uft_years:
        uft_annual = np.array([np.mean([uft_by_ym[(y, m)] for m in range(1, 13)]) for y in uft_years])
        print(f"unfinetuned: {len(uft_years)} complete years  [{uft_years[0]}-{uft_years[-1]}]")
    else:
        uft_years = None

# ---------- unfinetuned, interactive-ice global-mean SST, its own available full years ----------
uftloop_years, uftloop_annual = None, None
uftloop_files = sorted(glob.glob(f"{args.run_uft_loop}/*.pop.h.[0-9][0-9][0-9][0-9]-[0-9][0-9].nc"))
if uftloop_files:
    uftloop_keys = [ym(f) for f in uftloop_files]
    uftloop_by_ym = {k: gmean(xr.open_dataset(f)["TEMP"].isel(time=0, z_t=0).values)
                     for f, k in zip(uftloop_files, uftloop_keys)}
    uftloop_years_avail = sorted({y for (y, m) in uftloop_keys})
    uftloop_years = [y for y in uftloop_years_avail if all((y, m) in uftloop_by_ym for m in range(1, 13))]
    if uftloop_years:
        uftloop_annual = np.array([np.mean([uftloop_by_ym[(y, m)] for m in range(1, 13)]) for y in uftloop_years])
        print(f"unfinetuned+ice-loop: {len(uftloop_years)} complete years  [{uftloop_years[0]}-{uftloop_years[-1]}]")
    else:
        uftloop_years = None
...
# ---------- CAM6 (CREDIT training truth) global-mean SST, same years ----------
# Native CAM f09 lat-lon grid; weight = cos(lat) * (1-LANDFRAC) (ocean-fraction
# area weight, the f09 analog of gx1v7's TAREA mask -- includes ice-covered
# ocean, matching how the model/LE2 series are weighted above). This is the
# SST boundary condition our atmosphere emulator trains against, mirroring how
# the CAMulator paper always shows CAM6 alongside its own model's output.
...
ax.plot(full_years, m_annual, color=C_SIM, lw=2.4, solid_capstyle="round",
        ...
        label=f"Finetuned (sensreg) sim  (mean {m_annual.mean():.2f} °C)")
...
for i, y in enumerate(full_years):
    ...
    if cam_annual is not None:
        line += f"  cam6 {cam_annual[i]:6.3f}"
    print(line)
if uft_years is not None:
    print("unfinetuned:")
    for y, v in zip(uft_years, uft_annual):
        print(f"  {y}  uft {v:6.3f}")
if uftloop_years is not None:
    print("unfinetuned+ice-loop:")
    for y, v in zip(uftloop_years, uftloop_annual):
        print(f"  {y}  uft-loop {v:6.3f}")
