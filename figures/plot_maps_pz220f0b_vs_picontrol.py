"""Spatial SST quality maps: FASST production run (pz220f0b, unfinetuned,
interactive ice) vs CESM2 piControl "truth" vs error, for global + several
regions.

RE-RUNNABLE BY DESIGN: every parameter below is derived from whatever data
exists on disk right now, not hardcoded to a fixed calendar window. Re-run
this script at any point as the production chain (job 7255561 -> ...)
continues and it will automatically:
  - use ALL production months available,
  - re-detect the post-transient (stabilization) year from the production
    run's own annual global-mean SST trajectory (same trailing-slope method
    as make_fasst_neurips_2panel.py), so the averaging window grows and
    the excluded spin-up period never contaminates the maps,
  - pull as many consecutive piControl 100-yr blocks as needed to cover an
    ELAPSED-MODEL-YEAR-matched piControl window (piControl has no real
    calendar date -- see below -- so "matching 2000-2045" would be
    meaningless; what's matched is calendar-independent elapsed year count).

ELAPSED-YEAR ALIGNMENT (the actual fix from the previous, hardcoded version):
Production months are labeled with a fictitious calendar (the SOLIN forcing
file's climatology happens to be tagged "1980" internally), while piControl
is its own independent 1200-yr run branched from a 1850 spin-up with no
shared calendar. The only fair comparison is by ELAPSED MODEL YEAR since each
run's own start: production elapsed year E = calendar_year - 1980 + 1; the
matched piControl window is piControl's OWN elapsed years E_lo..E_hi (i.e.
the same number of years after ITS start), not the same absolute year index
count starting from piControl year 1 regardless of where production's window
sits. This matters because piControl also carries a slow secular drift within
its 1200 years (mean SST rises from ~18.10 degC in yr 1-100 to ~18.35 degC in
yr 1101-1200), so a fixed offset error changes the piControl baseline used.

GRID CHECK: both datasets are POP output on the same gx1v7 384x320 tripole
grid -- verified bit-identical TLAT/TLONG/KMT every run (cheap, always
re-checked, never assumed) before any regrid/diff. Both are then regridded
with the SAME ScatterToRegular kNN instance (built once, applied to both) onto
a common 0.5-deg lat/lon grid, so the two fields land on IDENTICAL destination
pixels before differencing.

Outputs (overwritten in place on every re-run -- this is meant to always
reflect "latest state of the production run", not to preserve history):
  output/maps_pz220f0b_vs_picontrol.pdf         (vector, publication quality)
  output/maps_pz220f0b_vs_picontrol_stats.txt   (bias/RMSE per region, provenance)

Usage: python plot_maps_pz220f0b_vs_picontrol.py
       [--window 15] [--slope-thresh 0.02] [--persist 5]   (stabilization detector, matches the 2-panel script)
"""
...
ap.add_argument("--min-model-year", type=int, default=None,
                 help="override the auto-detected stabilization year (elapsed model year, "
                      "1-based) -- e.g. 31 to match the 2-panel figure's spin-up cutoff")
ap.add_argument("--max-model-year", type=int, default=None,
                 help="cap the window's end (elapsed model year, 1-based) -- e.g. 140; "
...
# =====================================================================
# 1) discover ALL production months currently on disk
# =====================================================================
mfiles = sorted(glob.glob(f"{PZ_DIR}/*.pop.h.[0-9][0-9][0-9][0-9]-[0-9][0-9].nc"))
if not mfiles:
    raise SystemExit(f"no production monthly files found in {PZ_DIR}")
...
print(f"production: {len(mfiles)} months on disk, {len(full_years)} complete years "
...
print("GRID CHECK: production and piControl TLAT/TLONG/KMT are identical -- safe to regrid once, reuse.")
...
# =====================================================================
# 3) auto-detect the post-transient (stabilization) year from production's
#    own annual global-mean SST trajectory -- same method as the 2-panel
#    NeurIPS figure, so the two figures never disagree about the window.
# =====================================================================
msst_by_ym = {}
...
if args.max_model_year is not None:
    ...
    if YEAR_MAX > int(full_years[-1]):
        raise SystemExit(f"--max-model-year {args.max_model_year} (calendar year {YEAR_MAX}) "
                          f"exceeds available production data (latest complete year "
                          f"{full_years[-1]}, elapsed model yr {full_years[-1]-pz_year0+1})")
    print(f"--max-model-year {args.max_model_year}: capping window end")
else:
    YEAR_MAX = int(full_years[-1])
print(f"post-transient window: production years {YEAR_MIN}-{YEAR_MAX} "
...
if months_have < pi_month_hi:
    raise SystemExit(f"piControl only has {months_have} months available; need "
                      f"{pi_month_hi} to match production's elapsed window "
                      f"(years {YEAR_MIN}-{YEAR_MAX}, elapsed {elapsed_lo+1}-{elapsed_hi_months}). "
                      f"Add more piControl blocks or shorten the production window.")
...
# ---------- production time-mean SST (native grid, then regrid once) ----------
...
# =====================================================================
# 5) figure: rows = regions, cols = [truth (piControl), predicted (FASST), error]
# =====================================================================
...
for r, (name, (lat0, lat1), (lon0, lon1)) in enumerate(REGIONS):
    ...
    COL_TITLES = ["CESM2 piControl (truth)", "FASST (predicted)", "Error (FASST − truth)"]
    C2K = 273.15
...
fig.suptitle("FASST vs. CESM2 piControl Sea Surface Temperature "
             f"{YEAR_MIN-pz_year0+1}-{YEAR_MAX-pz_year0+1}yr",
             fontsize=18, fontweight="bold", y=0.995)
fig.tight_layout(rect=[0, 0, 1, 0.97])
out_pdf = "output/maps_pz220f0b_vs_picontrol.pdf"
fig.savefig(out_pdf)
...
out_txt = "output/maps_pz220f0b_vs_picontrol_stats.txt"
with open(out_txt, "w") as fh:
    fh.write(f"FASST vs CESM2 piControl SST -- regional bias/RMSE\n")
    fh.write(f"production months on disk : {len(mfiles)}\n")
