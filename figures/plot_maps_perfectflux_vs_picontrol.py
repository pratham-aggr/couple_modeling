"""Spatial SST quality maps: pz220f0b PERFECT-DOWNWELLING-FLUX run (--credit_fsds --credit_flds,
memo_pop_standalone_gx1v7_diagfw_perfectflux, perpetual GPU chain) vs CESM2
...
production run vs piControl) for the perfectflux experiment -- same regridding
...
Outputs (overwritten in place on every re-run):
  output/perfectflux_experiment/maps_perfectflux_vs_picontrol.pdf
  output/perfectflux_experiment/maps_perfectflux_vs_picontrol_stats.txt
...
PZ_DIR = "/glade/derecho/scratch/praggarwal/memo_pop_standalone_gx1v7_diagfw_perfectflux/run"
...
# =====================================================================
# 1) discover ALL perfectflux months currently on disk
# =====================================================================
mfiles = sorted(glob.glob(f"{PZ_DIR}/*.pop.h.[0-9][0-9][0-9][0-9]-[0-9][0-9].nc"))
if not mfiles:
    raise SystemExit(f"no perfectflux monthly files found in {PZ_DIR}")
...
print(f"perfectflux: {len(mfiles)} months on disk [{mkeys[0][0]}-{mkeys[0][1]:02d} to "
...
print("GRID CHECK: perfectflux and piControl TLAT/TLONG/KMT are identical -- safe to regrid once, reuse.")
...
# =====================================================================
# 3) auto-detect the post-transient (stabilization) year from perfectflux's
...
print(f"post-transient window: perfectflux years {YEAR_MIN}-{YEAR_MAX} "
...
if months_have < pi_month_hi:
    raise SystemExit(f"piControl only has {months_have} months available; need "
                      f"{pi_month_hi} to match perfectflux's elapsed window "
...
# ---------- perfectflux time-mean SST (native grid, then regrid once) ----------
...
# =====================================================================
# 5) figure: rows = regions, cols = [truth (piControl), predicted (perfectflux), error]
# =====================================================================
...
for r, (name, (lat0, lat1), (lon0, lon1)) in enumerate(REGIONS):
    ...
    COL_TITLES = ["CESM2 piControl (truth)", "perfectflux (predicted)", "Error (perfectflux − truth)"]
    C2K = 273.15
...
fig.suptitle("pz220f0b (--credit_fsds --credit_flds, prescribed real downwelling radiation) vs. CESM2 piControl Sea Surface Temperature "
             f"{YEAR_MIN-pz_year0+1}-{YEAR_MAX-pz_year0+1}yr",
             fontsize=18, fontweight="bold", y=0.995)
fig.tight_layout(rect=[0, 0, 1, 0.97])
out_pdf = "output/perfectflux_experiment/maps_perfectflux_vs_picontrol.pdf"
fig.savefig(out_pdf)
...
out_txt = "output/perfectflux_experiment/maps_perfectflux_vs_picontrol_stats.txt"
with open(out_txt, "w") as fh:
    fh.write(f"perfectflux vs CESM2 piControl SST -- regional bias/RMSE\n")
    fh.write(f"perfectflux months on disk : {len(mfiles)}\n")
